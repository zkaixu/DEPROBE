#!/bin/bash
# ==============================================================================
# DEPROBE-DNA: NIST7035 holdout validation data setup
# ==============================================================================
# Downloads BAM via aria2c over HTTPS with multi-part transfer and post-download
# integrity checks, extracts candidate probes, computes 12D thermodynamic priors,
# builds HDF5 dataset for blind holdout validation.
# ==============================================================================
...
STAGE_DIR="${DATA_DIR}/data_factory/staging/val"
FINAL_DIR="${DATA_DIR}/data_factory/final/val"
TEMP_DIR="${DATA_DIR}/temp_processing/val"
META_DIR="${DATA_DIR}/metadata"

DNA_STAGE_CSV="${STAGE_DIR}/deprobe_val_master.csv"
DNA_FINAL_H5="${FINAL_DIR}/deprobe_val_master.h5"
...
# Experimental data (NIST7035, validation)
BAM_NEXTERA="${BAM_DIR}/project.NIST_NIST7035_H7AP8ADXX_TAAGGCGA_2_NA12878.bwa.markDuplicates.bam"
# AWS S3 download route
BAM_S3_URI="s3://giab/data/NA12878/Garvan_NA12878_HG001_HiSeq_Exome/project.NIST_NIST7035_H7AP8ADXX_TAAGGCGA_2_NA12878.bwa.markDuplicates.bam"
BAI_S3_URI="s3://giab/data/NA12878/Garvan_NA12878_HG001_HiSeq_Exome/project.NIST_NIST7035_H7AP8ADXX_TAAGGCGA_2_NA12878.bwa.markDuplicates.bai"
# Fallback Route: HTTPS (NCBI Mirror)
BAM_HTTPS_URL="https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/data/NA12878/Garvan_NA12878_HG001_HiSeq_Exome/project.NIST_NIST7035_H7AP8ADXX_TAAGGCGA_2_NA12878.bwa.markDuplicates.bam"
BAI_HTTPS_URL="https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/data/NA12878/Garvan_NA12878_HG001_HiSeq_Exome/project.NIST_NIST7035_H7AP8ADXX_TAAGGCGA_2_NA12878.bwa.markDuplicates.bai"
...
# ------------------------------------------------------------------------------
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/val_factory_$(date +'%Y%m%d_%H%M%S').log"

echo "===================================================" > "$LOG_FILE"
echo "DEPROBE-DNA VAL PIPELINE LOG INITIALIZED AT $(date)" >> "$LOG_FILE"
echo "===================================================" >> "$LOG_FILE"
...
clear
echo -e "${CYAN}${BOLD}============================================================"
echo "      DEPROBE-DNA: NIST7035 validation data setup"
echo "============================================================${NC}"
...
log_info "Preparing NIST7035 validation alignment assets..."

if [ ! -f "$BAM_NEXTERA" ]; then
    log_warning "Downloading NIST7035 BAM..."
    ...
    log_success "NIST7035 alignment assets prepared."
else
    log_info "NIST7035 BAM already exists. Verifying integrity..."
    if ! samtools quickcheck -v "$BAM_NEXTERA" >> "$LOG_FILE" 2>&1; then
...
cat <<EOF > "${META_DIR}/metadata_val.csv"
Sample_Name,BAM_Path,BED_Path,Platform_ID
NA12878_Nextera_7035,${BAM_NEXTERA},${BED_NEXTERA},1
EOF
...
python3 build_pretrain_dataset.py \
    --meta "${META_DIR}/metadata_val.csv" \
    --ref "$REF_FASTA" \
    --mode dna \
    --out "$DNA_STAGE_CSV" \
    --temp "$TEMP_DIR" \
...
log_success "VAL Staging completed."
...
if [ -f "$DNA_FINAL_H5" ]; then
    log_success "Validation HDF5 location: ${DNA_FINAL_H5}"
else
